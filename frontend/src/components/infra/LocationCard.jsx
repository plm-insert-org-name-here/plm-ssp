...
const LocationCard = ({location, onAttach, onDetach, onDelete, onRename}) => {
    ...
    const onAttachPopupOpen = () => {
        axios(`${Routes.detectors}/attachable`)
            .then(res => {
    ...
    return (
        <>
                ...
                >
                    ...
                    <Box display="flex" alignItems="center">
                        {location.detector
                            ...
                            : <VideocamOffIcon
                                sx={{height: '32px', color: "text.lightest"}}/>}
                        <OverflowText color="text.lighter" text={location.detector?.name ?? ""}
            ...
            <ConfirmPopup
                ...
                text={<>Detaching <i>{location.detector?.name}</i></>}
            />
            <MenuPopup
                popupProps={attachPopup}
                items={attachPopupItems}
                handler={onAttachInner}
            />
            <ConfirmPopup
                popupProps={deletePopup}
                handler={onDeleteInner}
                text={<>Deleting <i>{location.name}</i></>}
            />
        </>
    );
