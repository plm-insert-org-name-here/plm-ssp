...
using Application.Services.ProcessorHandler.Packets.Req;
...
namespace Application.Services.MonitoringHandler
{
    ...
    public class MonitoringHandler
    {
        ...
        private void SubmitEvent(TaskInstance ins, Step change, string? failureReason = null)
        {
            var ev = new Event
            {
            ...
            TaskInstance ins)
        {
            ...
            if (newState is TemplateState.UnknownObject &&
                ...
                SubmitEvent(ins, step, $"Detected an unknown object at template (name: '${step.Template.Name}')");

            if (history.PreviousValidState is null)
            {
                if ((newState is TemplateState.Missing && step.ExpectedInitialState is TemplateState.Present) ||
                    (newState is TemplateState.Present && step.ExpectedInitialState is TemplateState.Missing))
                    SubmitEvent(ins, step, $"Incorrect initial state for template (name: '${step.Template.Name}')");

                UpdateTemplateStateHistory(history, newState);
                return false;
            }

            if (history.PreviousValidState == step.ExpectedInitialState
                && newState == step.ExpectedSubsequentState)
            {
                SubmitEvent(ins, step);
                ret = true;
            }
